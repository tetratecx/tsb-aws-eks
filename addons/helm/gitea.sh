...
function gitea_deploy_helm {
  ...
  if $(helm status gitea --kube-context "${cluster_context}" --namespace "${namespace}" &>/dev/null); then
...
function gitea_get_http_url {
  ...
  local gitea_hostname=$(kubectl get svc --context "${cluster_context}" --namespace "${namespace}" gitea-http -o jsonpath='{.status.loadBalancer.ingress[0].hostname}' 2>/dev/null) ;
